...
export const getProperties = async (
  ...
  try {
    ...
    if (sqftMin) {
      whereConditions.push(Prisma.sql`p."squareFeet" >= ${Number(sqftMin)}`);
    }

    if (sqftMax) {
      whereConditions.push(Prisma.sql`p."squareFeet" <= ${Number(sqftMax)}`);
    }
...
export const createProperty = async (
  ...
  try {
    ...
    const geocodingResponse = await axios.get(geocodingURL, {
      headers: {
        "User-Agent": "RealEstateApp/1.0 (Douglas Eduardo)",
      },
    });
    const geocodingData = geocodingResponse.data[0] || {};

    const latitude = parseFloat(geocodingData.lat) || 0;
